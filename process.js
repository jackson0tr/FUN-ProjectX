class QuestionManager {
    ...
    saveToLocalStorage(decodedAnswers, sessionId) {
  ...
  class App {
    ...
    decodeAnswersFromUrl(encodedUrl) {
      try {
        const urlParams = new URLSearchParams(window.location.search);
        const encodedData = urlParams.get('data'); // Assuming "data" is your key
        if (!encodedData) throw new Error('Encoded data not found');
  
        const decodedData = atob(encodedData); // Decode Base64
        return JSON.parse(decodedData); // Parse JSON
      } catch (error) {
        console.error('Failed to decode URL answers:', error);
        return null;
      }
    }
