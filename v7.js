...
class App {
    constructor(questionManager) {
        ...
        this.isLoggedIn = false; // Add state for login
    }
    ...
    start() {
        if (!this.isLoggedIn) {
            UIManager.renderFirebaseLogin(); // Show login form first
        } else {
            UIManager.renderGenderSelection(); // Show questions after login
        }
    }
